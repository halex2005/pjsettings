...
public:
     ...
     */
    void        writeNumber(const string &name, double num) throw(Error);
...
struct container_node_op
{
    ...
    ContainerNode   (*writeNewContainer)(ContainerNode*, const string &name) throw(Error);
    ContainerNode   (*writeNewArray)    (ContainerNode*, const string &name) throw(Error);
};
...
#define NODE_WRITE_FLOAT(node,item)    node.writeNumber(#item, (double)item)
